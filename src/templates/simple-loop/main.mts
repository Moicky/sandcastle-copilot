...
await run({
  ...
  // starts. This avoids a full {{PACKAGE_MANAGER}} install from scratch on every iteration.
  // The onSandboxReady hook still runs {{INSTALL_COMMAND}} as a safety net to handle
  ...
  hooks: {
    sandbox: {
      ...
      onSandboxReady: [{ command: "{{INSTALL_COMMAND}}" }],
    },
  },
});
